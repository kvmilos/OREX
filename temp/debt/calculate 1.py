...
OUTPUT_DIRECTORY = 'arkusze_wyjsciowe/'
INPUT_DIRECTORY = 'arkusze_wejsciowe/'
MISSING_RESERVATIONS_DIR = 'history_missing_reservations/'
...
def analyse_data(df, df_prev, date):
...
def classify_rows(df):
    ...
    conditions = [
        # Unpaid advances
        (df['Advance date'] < today) & (today < df['Date total']) & (df['Paid'] < df['Advance amount']) & (df['Days to check-in after confirmation'] > 30),
        # 30/30
        (df['Check-in days'] > 30) & (df['Overpayment of 30/30'] < 0),
        # Overdue receivables
        ((df['Date total'] <= today) & (today < df['Check-in date']) & (df['Debt'] > 0) & (df['Days from confirmation'] >= 3)) | ((df['Days to check-in after confirmation'] <= 30) & (today < df['Check-in date']) & (df['Debt'] > 0) & (df['Days from confirmation'] >= 3)),
        # Payables in term
...
def update_phone(df, phone_dict):
    df['NIP'] = df['NIP'].astype(str)
    df['Mobile phones'] = df['NIP'].map(phone_dict).fillna(df['Mobile phones'])
    return df
...
def process_excel_file(file_path, df_prev):
    ...
    dict_update_phone = {
        ...
        '5731110935': '693620630', #E-lastminute.pl
        '5862403922': '516704402' #Fiesta sp. z.o.o
    ...
    df = update_phone(df, dict_update_phone)
    ...
    df = analyse_data(df, df_prev, date)
...
if __name__ == "__main__":
    ...
    print(prev_file)
    # prev_file = 'C:\\analyze\\calculate-classification\\arkusze_wyjsciowe\\output_0103 DEBT COLLECTION REPORT.xlsx' # Dni wolne
